#tkinter,datetime,tkcalendar added for GUI purposes
#sqlite3 added to store input in database
import datetime
from tkinter import *
...
connector.execute(
"CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PASSWORD TEXT, GENDER TEXT, DOB TEXT, MAJOR TEXT)"
)
# Creating the functions
def reset_fields():
   global name_strvar, email_strvar, password_strvar, gender_strvar, dob, major_strvar
   for i in ['name_strvar', 'email_strvar', 'password_strvar', 'gender_strvar', 'major_strvar']:
       exec(f"{i}.set('')")
...
def add_record():
   ...
   DOB = dob.get_date()
   major = major_strvar.get()
   if not name or not email or not password or not gender or not DOB or not major:
       mb.showerror('Error!', "Please fill all the missing fields!!")
   else:
       try:
           connector.execute(
           'INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, PASSWORD, GENDER, DOB, MAJOR) VALUES (?,?,?,?,?,?)', (name, email, password, gender, DOB, major)
           )
           connector.commit()
...
def view_record():
   ...
   if not tree.selection():
       mb.showerror('Error!', 'Please select a record to view')
        ...
        password_strvar.set(selection[3]); gender_strvar.set(selection[4])
        date = datetime.date(int(selection[5][:4]), int(selection[5][5:7]), int(selection[5][8:]))
        dob.set_date(date);major_strvar.set(selection[6])
...
Label(left_frame, text="Email Address", font=labelfont, bg=lf_bg).place(relx=0.175, rely=0.18)
Label(left_frame, text="Password", font=labelfont, bg=lf_bg).place(relx=0.2, rely=0.31)
...
Label(left_frame, text="Major", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.7)
...
Entry(left_frame, width=19, textvariable=password_strvar, font=entryfont).place(x=20, rely=0.36)
Entry(left_frame, width=19, textvariable=major_strvar, font=entryfont).place(x=20, rely=0.75)
...
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                   columns=('Student ID', "Name", "Email Address", "Password", "Gender", "Date of Birth", "Major"))
...
tree.heading('Major', text='Major', anchor=CENTER)
